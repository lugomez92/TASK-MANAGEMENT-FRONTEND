...
const TaskModal = ({ taskId, onClose, token, onTaskUpdated, onTaskDeleted, setMessage, role }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const taskResponse = await axios.get(`${process.env.REACT_APP_API_URL}/api/tasks/${taskId}`, {
        ...
        const engineersResponse = await axios.get(`${process.env.REACT_APP_API_URL}/api/teams/${taskResponse.data.task.teamId}/engineers`, {
          headers: { Authorization: `Bearer ${token}` }
        });
