...
const EngineerBoard = ({ token, engineerId, teamId, onCardClick, fetchTasks }) => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/tasks/engineer/${engineerId}`, {
    ...
    fetchTasks();
  }, [token, engineerId, teamId, fetchTasks]);
